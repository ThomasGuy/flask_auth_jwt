...
class TestAuthBlueprint(BaseTestCase):
    ...
    def test_registration(self):
        ...
        with self.client:
            ...
            self.assertFalse(is_token_revoked(
                decode_token(data_register['access_token'])))
    ...
    def test_registered_with_already_registered_user(self):
        ...
        with self.client:
            ...
            self.assertTrue(data['message'] ==
                            'User/email already exists. Please Log in.')
            self.assertTrue(response.content_type == 'application/json')
            self.assertEqual(response.status_code, 202)
            response = self.register_user(
                username='wrong', email='test@gmail.com')
            data = json.loads(response.data.decode())
            self.assertTrue(data['status'] == 'fail')
            self.assertTrue(data['message'] ==
                            'User/email already exists. Please Log in.')
    ...
    def test_registered_user_login_username(self):
        ...
        with self.client:
            # user registration
            resp_register = self.register_user(
                username='bobby', email='joe@gmail.com', password='12345678')
            data_register = json.loads(resp_register.data.decode())
            self.assertTrue(data_register['status'] == 'success')
            self.assertTrue(data_register['access_token'])
            self.assertTrue(resp_register.content_type == 'application/json')
            ...
            response = self.login_user(
                username='bobby', email=None, password='12345678')
            data = json.loads(response.data.decode())
    ...
    def test_non_registered_user_login(self):
        ...
        with self.client:
            response = self.login_user(
                username='tom', email='twg@gmail.com', password='1989123')
            data = json.loads(response.data.decode())
            self.assertTrue(data['status'] == 'fail')
    ...
    def test_incorrect_password_user_login(self):
        ...
        with self.client:
            ...
            response = self.login_user(
                username='joeseph', email='joe@gmail.com', password='wrongpasswd')
            data = json.loads(response.data.decode())
            self.assertTrue(data['status'] == 'fail')
            self.assertTrue(data['message'] == 'User does not exist.')
            self.assertTrue(response.content_type == 'application/json')
    ...
    def test_refresh_expired_token(self):
        ...
        with self.client:
            ...
            self.assertFalse(is_token_revoked(
                decode_token(data_refresh['access_token'])))
            response = self.client.get(
                '/protected',
                headers=dict(
    ...
    def test_user_status(self):
        ...
        with self.client:
            resp_register = self.register_user(
                username='Thomas', email='tom@mail.box', password='ahoy')
            data_register = json.loads(resp_register.data.decode())
            self.assertTrue(data_register['status'] == 'success')
            self.assertTrue(data_register['access_token'])
            self.assertTrue(resp_register.content_type == 'application/json')
            self.assertEqual(resp_register.status_code, 201)
            resp_login = self.login_user(
                username='Thomas', email='', password='ahoy')
            data_login = json.loads(resp_login.data.decode())
            self.assertTrue(data_login['status'] == 'success')
            self.assertTrue(data_login['access_token'])
            self.assertTrue(resp_login.content_type == 'application/json')
    ...
    def test_using_revoked_access_token(self):
        ...
        with self.client:
            ...
            # user login
            resp_login = self.login_user(
                username='Tom', email=None, password='123456')
            data_login = json.loads(resp_login.data.decode())
            self.assertTrue(data_login['status'] == 'success')
            self.assertTrue(data_login['access_token'])
            self.assertTrue(resp_login.content_type == 'application/json')
